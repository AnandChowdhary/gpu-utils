...
VARIANT_PHRASES: dict[str, list[str]] = {
    k: v[:MAX_VARIANT_PHRASES] for k, v in VARIANTS.items()
}
...
VAR_RULES: dict[str, object] = {
    ...
    "strong": [
        [
            "group-focus",
            ["group", "parent", "container", "ancestor", "wrapper"],
            ["focus", "focused", "focussed", "focusing"],
        ],
        [
            "group-hover",
            ["group", "parent", "container", "ancestor", "wrapper"],
            ["hover", "hovered", "hovering", "moused", "mouseover"],
        ],
        ["dark", ["dark", "night", "darkmode"], []],
        [
            "focus-visible",
            ["keyboard", "visible", "tabbed"],
            ["focus", "focused", "focussed", "tabbed"],
        ],
        [
            "focus-within",
            ["within", "child", "inside", "descendant", "inner", "input"],
...
def words_of(text: str) -> list[str]:
    ...
    return [
        normalize_word(w) for w in re.findall(r"[a-z0-9]+|[^\sa-z0-9]", text.lower())
    ]
...
def resolve_variant(words: list[str]) -> str | None:
    ...
    if "very" in ws and "small" in ws:
        tier = "sm"
    up_to = any(
        words[i] == "up" and words[i + 1] == "to" for i in range(len(words) - 1)
    )
