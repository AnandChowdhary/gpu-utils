/**
 * gpu-email: Split plain-text emails into reply, quoted history, and signature, and extract contact details
 *
...
import { type GpuEmailResult, decode } from "./decode.ts";
import { featurize } from "./features.ts";
import { forwardGpu } from "./gpu.ts";
import { MODEL } from "./model.ts";

export type { GpuEmailResult } from "./decode.ts";

export interface GpuEmailOptions {
...
export async function parse(
  text: string,
  options: GpuEmailOptions = {},
): Promise<GpuEmailResult> {
