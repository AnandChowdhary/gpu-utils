# gpu-email

Split plain-text emails into reply, quoted history, and signature, and extract contact details

Tiny model, trained from scratch, runs on WebGPU in the browser. Zero dependencies.
Part of [gpu-utils](https://github.com/AnandChowdhary/gpu-utils).

```bash
npm install gpu-email
```

```ts
import { parse } from "gpu-email";

const result = await parse("...");
```

## How it works

TODO: tokenizer → sparse features → model → decoder, in three sentences.

## Size and speed

TODO: table from `pnpm size` and the benchmark.

## Limitations

TODO. See [MODEL_CARD.md](./MODEL_CARD.md).

## Training

```bash
cd packages/gpu-email/training
uv sync
uv run python -m gpu_email.data      # build/generate dataset
uv run python -m gpu_email.train     # train with QAT
uv run python -m gpu_email.export    # write ../model/{manifest.json,weights.txt,fixtures.json}
```
