...
describe("argmax", () => {
	it("picks the max per row", () => {
		expect(
			Array.from(argmax(new Float32Array([0, 1, 2, 5, 3, 1]), 2, 3)),
		).toEqual([2, 0]);
	});
});
...
describe("viterbi", () => {
	...
	it("handles empty input", () => {
		expect(viterbi(new Float32Array(0), 0, 2, new Float32Array(4)).length).toBe(
			0,
		);
	});
});
...
describe("decodeInt6", () => {
	it("matches the Python encoder for [0.5, -0.25, 0, 1.0]", () => {
		...
		const encoded =
			alphabet[48]! + alphabet[24]! + alphabet[32]! + alphabet[63]!;
		const tensors: TensorEntry[] = [
			{ name: "w", offset: 0, length: 4, shape: [4], scale: 1 / 31 },
		];
		const w = decodeInt6(encoded, tensors);
		expect(Array.from(w).map((v) => Math.round(v * 1000) / 1000)).toEqual([
			0.516, -0.258, 0, 1,
		]);
	});
});
