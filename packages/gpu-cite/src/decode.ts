...
const ARXIV_VALUE = /^(?:\d{4}\.\d{4,5}(?:v\d+)?|[a-z-]+(?:\.[A-Za-z]{2})?\/\d{7}(?:v\d+)?)$/i;
...
function softmaxAt(v: Float32Array, offset: number, k: number, index: number): number {
  let max = -Infinity;
...
export function decode(
  ...
  const entSpan = (e: Entity): Span => trimSpan(text, tokens[e.first]!.start, tokens[e.last]!.end);
