...
const FLAG = Object.fromEntries(FLAG_NAMES.map((n, i) => [n, i])) as Record<
	FlagName,
	number
>;
...
group(
	"CUE_THESIS",
	"thesis dissertation phd ph msc ma master masters doctoral diss",
);
group(
	"CUE_REPORT",
	"report technical tech rep memo memorandum working paper tr rfc",
);
group(
	"CUE_ACCESS",
	"retrieved accessed available viewed visited cited online from",
);
group("CUE_DOI", "doi");
...
group(
	"CUE_WEB",
	"web website homepage blog wikipedia github http https www html htm com org net",
);
...
export function findUrls(text: string): Span[] {
	...
	for (const m of text.matchAll(URL_RE))
		out.push(trimMatch(text, m.index, m.index + m[0].length));
	return out;
}
