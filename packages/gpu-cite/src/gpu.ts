...
export async function forwardGpuBatch(model: Model, batch: FeatureRows[]): Promise<Logits[]> {
  ...
  const stateSizes = [N * E, N * C * 2, N * C, N * C, N * C * 2, N * C, S * 2 * C, N * HEAD, S * H];
  let stateTotal = 0;
  ...
  const state = program.buffer("state", new Float32Array(Math.max(1, stateTotal)));
  ...
  const groups = (count: number): [number] => [Math.max(1, Math.ceil(count / 64))];
