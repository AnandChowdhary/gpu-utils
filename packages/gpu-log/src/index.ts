/**
 * gpu-log: Universal log line parser: timestamps, levels, sources, key=value pairs, messages, and stack frames from any format
 *
...
import { type GpuLogResult, decode } from "./decode.ts";
import { featurize } from "./features.ts";
import { forwardGpu } from "./gpu.ts";
import { MODEL } from "./model.ts";

export type { GpuLogResult } from "./decode.ts";

export interface GpuLogOptions {
...
export async function parse(
  text: string,
  options: GpuLogOptions = {},
): Promise<GpuLogResult> {
