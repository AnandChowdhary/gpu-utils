...
describe("gpu-view examples", () => {
	it("total revenue by region this quarter, top 10, as a bar chart", async () => {
		const spec = await parse(
			"total revenue by region this quarter, top 10, as a bar chart",
			{
				schema: sales,
				now,
				backend: "cpu",
	...
	it("average order value per month last year, line chart", async () => {
		const spec = await parse(
			"average order value per month last year, line chart",
			{
				schema: orders,
				now,
				backend: "cpu",
	...
	it("tolerates plurals, aliases and one-character typos in field names", async () => {
		const spec = await parse(
			"active custmers with orderz over 10k, signup after 2024",
			{
				schema: customers,
				now,
				backend: "cpu",
