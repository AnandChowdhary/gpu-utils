...
export function forwardCpu(model: Model, features: FeatureRows): Float32Array {
  ...
  for (let t = 0; t < n; t++) {
    for (const row of features.rows[t]!) {
      for (let c = 0; c < H; c++) emb[t * H + c] = emb[t * H + c]! + embedding[row * H + c]!;
    }
  }
  ...
  for (let t = 0; t < n; t++) {
    for (let c = 0; c < H; c++) {
      ...
      for (let k = 0; k < C; k++) {
        const src = t + k - half;
        if (src >= 0 && src < n) s += convolution[k * H + c]! * emb[src * H + c]!;
      }
  ...
  for (let t = 0; t < n; t++) {
    for (let c = 0; c < H; c++) {
      let s = combB[c]!;
      for (let i = 0; i < H; i++) {
        s += fwd[t * H + i]! * combW[c * 2 * H + i]! + bwd[t * H + i]! * combW[c * 2 * H + H + i]!;
      }
  ...
  for (let t = 0; t < n; t++) {
    ...
    for (let o = 0; o < outs; o++) {
      let s = outB[o]!;
      for (let h = 0; h < hidden.length; h++) s += hidden[h]! * outW[o * hidden.length + h]!;
      logits[t * outs + o] = s;
    }
  }
  return logits;
