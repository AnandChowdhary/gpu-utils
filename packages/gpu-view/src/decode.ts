...
export function decode(
	...
	const roles = Array.from(
		ids,
		(i) => (model.manifest.labels[i] ?? "O") as Role,
	);
...
const EMPTY_RE =
	/\b(without|missing|lacking|empty|blank|null|unset|none|unknown|nothing)\b/;
...
function defaultDateField(ctx: Ctx, span: Span): number {
	const dates = ctx.schema.fields
		.map((f, i) => (f.kind === "date" ? i : -1))
		.filter((i) => i >= 0);
	...
	if (dates.length === 0)
		diag(
			ctx,
			"no_date_field",
			"a time phrase was used but the schema has no date field",
			span,
		);
	else
		diag(
...
function compileFilter(ctx: Ctx, clause: number[]): undefined {
	...
	if (fieldRuns.length === 0 && valueRuns.length === 0 && timeRuns.length === 0)
		return;
	...
	if (fieldRuns.length > 0) {
		...
		for (const run of valueRuns) {
			...
			owners =
				owners === null
					? new Set(m.owners)
					: new Set(m.owners.filter((o) => owners!.has(o)));
		}
		if (owners?.size !== 1) {
			diag(
				ctx,
	...
	const negatedUnsupported = (op: FilterOp) => {
		if (negs % 2 === 1) {
			diag(
				ctx,
				"unsupported_negation",
				`a negated "${op}" filter is not supported`,
				span,
			);
			return true;
		}
	...
	if (field.kind === "date" && timeRuns.length === 0 && valueRuns.length > 0)
		timeRuns = valueRuns;
	const hasValues =
		field.kind === "date" ? timeRuns.length > 0 : valueRuns.length > 0;
	if (!hasValues) {
	...
	if (field.kind === "number") {
		...
		if (nums.length >= 2 || /\bbetween\b/.test(opText)) {
			if (nums.length < 2)
				return diag(ctx, "incomplete_range", "a range needs two numbers", span);
	...
	if (field.kind === "date") {
		...
		if (/\bago$/.test(runText(ctx, timeRuns[0]!).trim().toLowerCase()))
			fam =
				(
					{ lt: "gt", gt: "lt", lte: "gte", gte: "lte" } as Record<
						string,
						FilterOp
					>
				)[fam] ?? fam;
...
function dateOp(op: string): FilterOp | "in" {
	const has = (re: RegExp) => re.test(op);
	if (has(/\b(until|till|through|up to|by|or before|at most|no later)\b|<=|≤/))
		return "lte";
	if (has(/\b(before|prior|earlier|older|less|fewer|under|below)\b|</))
		return "lt";
	if (has(/\b(since|starting|from|or after|at least)\b|>=|≥/)) return "gte";
	if (has(/\b(after|later|newer|past|more|greater|over|above)\b|>/))
		return "gt";
	return "in";
}
...
function compileSort(ctx: Ctx, clause: number[]): void {
	...
	if (fieldRuns.length === 0) {
		...
		if (fi >= 0)
			ctx.spec.sort.push({ field: ctx.schema.fields[fi]!.name, dir, span });
		return;
	}
...
function compileGroup(ctx: Ctx, clause: number[]): void {
	for (const run of runs(ctx, clause, "GROUP_FIELD")) {
		...
		if (unit && ctx.schema.fields[m.field]!.kind === "date")
			ctx.spec.granularity = unit;
	}
}
...
function aggregateFn(t: string): AggregateFn {
	...
	if (/\b(max|maximum|highest|largest|biggest|peak|most)\b/.test(t))
		return "max";
	return "sum";
}
...
function compileAggregate(ctx: Ctx, clause: number[]): void {
	...
	for (const run of fieldRuns) {
		...
		const before = items.filter(
			(it) => it.at < run.start && it.field === undefined,
		);
		const target =
			before[before.length - 1] ??
			items.find((it) => it.at > run.start && it.field === undefined);
...
function compileLimit(ctx: Ctx, clause: number[]): void {
	...
	if (v === null || !Number.isInteger(v) || v <= 0) {
		diag(
			ctx,
			"invalid_limit",
			`"${runText(ctx, run)}" is not a valid limit`,
			runSpan(ctx, run),
		);
		return;
	}
...
function compileChart(ctx: Ctx, clause: number[]): void {
	...
	if (/\b(bar|bars|column|columns|histogram)\b/.test(t)) chart = "bar";
		...
		/\b(number|kpi|scorecard|stat|metric|tile|figure|value|single|big)\b/.test(
			t,
		)
	)
		chart = "number";
	if (chart === undefined) {
		diag(
			ctx,
			"unknown_chart",
			`"${t}" is not a known chart type`,
			runSpan(ctx, chartRuns[0]!),
		);
		return;
	}
