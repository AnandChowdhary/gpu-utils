 ...
 */
import { CharClass, type FeatureRows, hashToken, Shape } from "@gpu-utils/runtime";
...
const KIND_ID: Record<FieldKind, number> = { text: 1, number: 2, date: 3, enum: 4, boolean: 5 };
...
function lengthBucket(n: number): number {
  for (let i = 0; i < LENGTH_BUCKETS.length; i++) if (n <= LENGTH_BUCKETS[i]!) return i;
...
const distBucket = (d: number | null) => (d === null ? DIST_ROWS - 1 : Math.min(d, DIST_ROWS - 2));
...
export function featurize(text: string, schema: Schema): ViewFeatures {
  ...
  for (let i = n - 1; i >= 0; i--) {
    const own = fieldAt[i]!;
    nextSpan[i] = own === null || upcoming !== own ? upcoming : after(fields, own);
