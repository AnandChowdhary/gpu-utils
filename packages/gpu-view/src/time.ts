...
function period(unit: string, today: Date, shift: number): [Date, Date] {
  ...
  if (unit === "month") {
    const d = addMonths(utc(today.getUTCFullYear(), today.getUTCMonth() + 1, 1), shift);
...
export function resolveTime(text: string, today: Date): DateRange | null {
  ...
  if (!t) return null;
  const day = utc(today.getUTCFullYear(), today.getUTCMonth() + 1, today.getUTCDate());
...
function resolveNormalized(t: string, today: Date): [Date, Date] | null {
  ...
  if (t === "mtd" || t === "month to date") return [utc(y, today.getUTCMonth() + 1, 1), today];
  if (t === "qtd" || t === "quarter to date") return [period("quarter", today, 0)[0], today];
  ...
  m = /^(\d{4})-(\d{2})$/.exec(t);
  if (m && Number(m[2]) >= 1 && Number(m[2]) <= 12) return monthRange(Number(m[1]), Number(m[2]));
  ...
  if (m && UNITS[m[3]!]) {
    ...
    if (unit === "quarter") return [addDays(addMonths(today, -3 * n), 1), today];
  ...
  m = /^(this|current|last|past|previous|prior|next|coming|upcoming) (\w+)$/.exec(t);
  if (m && UNITS[m[2]!]) {
    const unit = UNITS[m[2]!]!;
    const w = m[1]!;
    const shift =
  ...
  if (m) {
    ...
    return m[1] === "1" ? [utc(yy, 1, 1), utc(yy, 6, 30)] : [utc(yy, 7, 1), utc(yy, 12, 31)];
  }
  m = /^([a-z]+)(?: (\d{4}))?$/.exec(t);
  if (m) {
