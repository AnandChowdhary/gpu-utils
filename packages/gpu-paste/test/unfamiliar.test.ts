...
describe("gpu-paste unfamiliar set", async () => {
	...
	for (const c of all) {
		...
		if (r.kind === c.kind) correct++;
			...
			misses.push(
				`${JSON.stringify(c.text.slice(0, 40))}: expected ${c.kind}, got ${r.kind}`,
			);
		const gold = new Set(
			c.spans.map((s) => {
		...
		const pred = new Set(
			r.spans.map((s) => `${s.kind}:${s.span[0]}:${s.span[1]}`),
		);
		for (const p of pred) {
		...
		for (const g of gold)
			if (!pred.has(g)) fn[g.split(":")[0]!] = (fn[g.split(":")[0]!] ?? 0) + 1;
	}
	const kinds = [
		...new Set([...Object.keys(tp), ...Object.keys(fp), ...Object.keys(fn)]),
	].sort();
