...
function buildParsed(kind: PasteKind, text: string, spans: RuleSpan[]): unknown {
  const pick = (k: string) =>
    spans.filter((s) => s.kind === k).map((s) => s.value ?? text.slice(s.span[0], s.span[1]));
  if (kind === "contact") {
  ...
  if (kind === "list") {
    const lines = text
      .split(/\r?\n/)
      .map((l) =>
        l.replace(/^\s*(?:[-*•·–—+>]+|\d+[.)]|[a-z][.)]|\[[ x]\]|[☐✅✓→]|\(\d+\))\s*/i, "").trim(),
      )
      .filter(Boolean);
    const items =
      lines.length > 1
  ...
  if (kind === "markdown") {
    return {
      headings: [...text.matchAll(/^#{1,6}\s+(.+)$/gm)].map((m) => m[1]!.trim()),
...
export async function parse(text: string, options: PasteOptions = {}): Promise<PasteResult> {
  ...
  const whole: [number, number] = [trimmedStart, trimmedStart + text.trim().length];
  ...
  if (rule && (rule.kind === "number" || rule.kind === "path" || rule.kind === "jwt")) {
    return {
      kind: rule.kind,
      confidence: rule.confidence,
      spans: [],
  ...
  const useGpu =
    backend === "webgpu" || (backend === "auto" && hasWebGPU() && tokens.length >= GPU_MIN_TOKENS);
  const model = await runModel(tokens, rows, useGpu);
  const spans = mergeSpans(text, model.spans);
  const probs = softmax(model.kind);
  const kindProbabilities = Object.fromEntries(MODEL.manifest.kinds.map((k, i) => [k, probs[i]!]));
  ...
  if (spans.length === 1 && (kind === "prose" || kind === "contact" || kind === "address")) {
    const s = spans[0]!;
    const covered = text.slice(s.span[0], s.span[1]).trim().length / trimmed.length;
    const promoted = PROMOTE[s.kind];
    if (promoted && covered >= 0.9 && promoted !== kind) {
      notes.push(`promoted ${kind} → ${promoted}: the whole paste is one ${s.kind} span`);
      kind = promoted;
