...
import { type GpuPasteResult, decode } from "./decode.ts";
import { featurize } from "./features.ts";
import { forwardGpu } from "./gpu.ts";
import { MODEL } from "./model.ts";

export type { GpuPasteResult } from "./decode.ts";

export interface GpuPasteOptions {
...
export async function parse(
  text: string,
  options: GpuPasteOptions = {},
): Promise<GpuPasteResult> {
