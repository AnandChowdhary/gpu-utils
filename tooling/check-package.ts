...
if (pkg.dependencies && Object.keys(pkg.dependencies).length > 0) {
	console.error(
		`${pkg.name}: published packages must have zero runtime dependencies`,
	);
	process.exit(1);
}
...
const [info] = JSON.parse(out) as [
	{ files: { path: string; size: number }[]; size: number },
];
const allowed =
	/^(dist\/[^/]+\.(js|d\.ts)|README\.md|MODEL_CARD\.md|LICENSE|package\.json)$/;
const bad = info!.files.filter((f) => !allowed.test(f.path));
for (const f of info!.files) console.log(`  ${f.path} (${f.size} B)`);
if (bad.length) {
	console.error(
		`${pkg.name}: unexpected files in tarball: ${bad.map((f) => f.path).join(", ")}`,
	);
	process.exit(1);
}
