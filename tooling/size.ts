...
export function measure(
	dir: string,
): { name: string; bytes: number; budget?: number } | null {
	...
	if (!existsSync(entry))
		throw new Error(`${pkg.name}: dist/index.js missing, run build first`);
	...
	if (pkg.gpuUtils?.sizeBudget !== undefined)
		result.budget = pkg.gpuUtils.sizeBudget;
	return result;
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
	...
	const dirs = all
		? readdirSync(resolve(root, "packages")).map((d) =>
